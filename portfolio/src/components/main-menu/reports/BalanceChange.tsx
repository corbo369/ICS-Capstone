/**
 * @file Balance Change Report Component
...
// Import Types
import { BalanceChangeReport } from "@/types/database.ts";

// Component Props
interface BalanceChangeProps {
  balanceChangeReport: BalanceChangeReport[];
}

const BalanceChange: React.FC<BalanceChangeProps> = ({balanceChangeReport}) => {
  // Sorting/filtering state
  const [sortField, setSortField] = useState<"firstDate" | "lastDate" | "change">("change");
  const [sortType, setSortType] = useState<"ASC" | "DESC">("DESC");
  const [filters, setFilters] = useState({ type: "", asset: "" });

  // Sorts the holdings list according to the active sort field
  const sortReport = [...balanceChangeReport]
    .sort((a, b) => {
      if (sortField === "firstDate") {
        ...
        const valueA = a.BalanceChange;
        const valueB = b.BalanceChange;
        return sortType === "DESC" ? valueB - valueA : valueA - valueB;
      }
    });

  // Handles changes to the active sort field
  const handleSort = (field: "lastDate" | "firstDate" | "change") => {
    if (sortField === field) {
  ...
  // Helper function for balance styling
  const getBalanceColor = (balance: number) => {
    if (balance > 0) return "text-green-300 text-right";
    if (balance < 0) return "text-red-300 text-right";
    return "text-gray-400 text-right";
  };

  // Helper function for balance multiplier
  const getBalanceMultiplier = (balance: number) => {
            ...
            <div className="flex flex-row items-center justify-center gap-1">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="dark w-48">
                  ...
                  <DropdownMenuCheckboxItem
                    checked={filters.type === "increase"}
                    onCheckedChange={() => setFilters(prev => ({
                      ...prev,
                      type: prev.type === "increase" ? "" : "increase"
                    }))}
                  >
                    Increase
                  </DropdownMenuCheckboxItem>
                  <DropdownMenuCheckboxItem
                    checked={filters.type === "decrease"}
                    onCheckedChange={() => setFilters(prev => ({
                      ...prev,
                      type: prev.type === "decrease" ? "" : "decrease"
                    }))}
                  >
                    Decrease
                  </DropdownMenuCheckboxItem>
                  ...
                  {Array.from(new Set(balanceChangeReport.map(t => t.Name))).map(asset => (
                    <DropdownMenuCheckboxItem
                      key={asset}
        ...
        {sortReport
          ...
          .map(r => (
            <TableRow className="h-15" key={r.Name}>
              ...
              <TableCell className={getBalanceColor(r.BalanceChange)}>
                {(getBalanceMultiplier(r.BalanceChange) * r.BalanceChange).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </TableCell>
            </TableRow>
          ))}
      </TableBody>
